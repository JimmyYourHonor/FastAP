...
def softBinning(D, mid, Delta):
    ...
    return torch.max(torch.Tensor([0]).to('cpu'), y)
...
class FastAP(torch.autograd.Function):
    ...
    def forward(ctx, input, target, num_bins):
        ...
        I_pos = (Aff > 0).type(torch.float).to('cpu')
        I_neg = (Aff < 0).type(torch.float).to('cpu')
        ...
        Delta = torch.tensor(4. / num_bins).to('cpu')
        Z     = torch.linspace(0., 4., steps=num_bins+1).to('cpu')
        L     = Z.size()[0]
        h_pos = torch.zeros((N, L)).to('cpu')
        h_neg = torch.zeros((N, L)).to('cpu')
        for l in range(L):
    ...
    def backward(ctx, grad_output):
        ...
        d_AP_h_pos = d_AP_h_pos + torch.mm(tmp1, LTM1.to('cpu'))
        ...
        d_AP_h_neg = torch.mm(tmp2, LTM0.to('cpu'))
